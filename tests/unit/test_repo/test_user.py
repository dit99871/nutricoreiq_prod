...
async def test_create_user_success(user_create_data):
    ...
    with patch("src.app.repo.user.get_password_hash", return_value=b"hashed_password"):
...
async def test_create_user_database_error(user_create_data):
    ...
    with patch("src.app.repo.user.get_password_hash", return_value=b"hashed_password"):
        with patch("src.app.repo.user.log") as mock_logger:
            with pytest.raises(HTTPException) as exc_info:
                user_create = UserCreate(**user_create_data)
...
async def test_update_user_password_success():
    ...
    with patch(
        "src.app.repo.user.get_password_hash", return_value=b"new_hashed_password"
    ):
...
async def test_update_user_password_hash_verification():
    ...
    with patch(
        "src.app.repo.user.get_password_hash", return_value=new_hash
    ) as mock_hash:
        await update_user_password(mock_session, "test-uid-123", "NewPassword123!")
...
async def test_update_user_password_with_empty_password():
    ...
    with patch(
        "src.app.repo.user.get_password_hash", return_value=b"empty_hash"
    ) as mock_hash:
